module Envy
  class CmdSwap
    ...
    def self.parse_tokens(args, opts)
      raise Envy::Error.new "swap: provide two names to swap. Use 'swap <name1> <name2>'" if args.size != 2
    ...
    def execute(ctx)
      ...
      raise Envy::Error.new "swap: no such var '#{ea}'" if !raw_a && !ctx.force?

      eb = ctx.expand_name(@name_b)
      raw_b = ctx.raw_get(eb)
      raise Envy::Error.new "swap: no such var '#{eb}'" if !raw_b && !ctx.force?
