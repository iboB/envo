module Envy
  class CmdListDel
    ...
    def execute(ctx)
      ...
      if list.type == :empty
        return if ctx.force?
        raise Envy::Error.new "list-del: deleting an item from a non-exiting value"
      end

      ok = list.list?
      ok ||= ctx.ask("#{ename} is not a list, but a #{list.type}. Convert?")
      raise Envy::Error.new "list-del: deleting an item from a non-list" if !ok

      list = list.to_list

      val = ctx.expand_value(@value)
      if val.list?
        raise Envy::Error.new "list-del: can't delete a list from a list" if !ctx.force?
        return
      end
      val = val.to_s
      ...
      if !ctx.raw? && index.to_s == val
        ...
        if !deleted
          raise Envy::Error.new "list-del: no index #{val} in #{ename}" if !ctx.force?
          return
        end
      else
        deleted = list.delete(val)
        if !deleted
          raise Envy::Error.new "list-del: no item '#{val}' in #{ename}" if !deleted
          return
        end
      end
